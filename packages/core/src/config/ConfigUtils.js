...
const VaultPermissions = require("./VaultPermissions");
...
class ConfigUtils {
  ...
  static async setSecrez(config, container, localWorkingDir) {
    ...
    await VaultPermissions.ensureDir(config.dataPath);
    await VaultPermissions.ensureDir(config.keysDataPath);
    await VaultPermissions.ensureDir(config.localDataPath);
    await VaultPermissions.ensureDir(config.tmpPath);
...
`,
      ...
      await VaultPermissions.writeFile(config.envPath, "{}");
    }
...
`,
      ...
      if (!/local/.test(gitignore)) {
        ...
        await VaultPermissions.writeFile(gitignorePath, gitignore, {
          encoding: "utf-8",
        });
      }
    }
    await VaultPermissions.ensureDir(container);
    return config;
  }
    ...
    if (
      ...
      if (!fs.existsSync(dataPath)) {
        ...
        if (
          ...
          fs.mkdirSync(dataPath, { mode: VaultPermissions.dirMode });
          fs.chmodSync(dataPath, VaultPermissions.dirMode);
        } else {
  ...
  static async putEnv(config, env) {
    return await VaultPermissions.writeFile(
      config.envPath,
      JSON.stringify(env)
    );
  }
